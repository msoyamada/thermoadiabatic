...
###############setup#########################
tempo_inicial=   1680#minutos
temperatura_pico= 81 #celsius
coeficiente_angular= 0.00056235
coeficiente_linear= (tempo_inicial*coeficiente_angular) + temperatura_pico


def calculaTemperaturaAlvo(ciclo):
    t= ciclo* 30
    temperatura_alvo= -(coeficiente_angular*t)+ coeficiente_linear
    return temperatura_alvo
...
# Arquivo que armazena em qual ciclo ele estperando, para preservar o
...
targetTemp=calculaTemperaturaAlvo(ciclo)
...
while (1):
	...
	if (maior_concreto <  (targetTemp- 1) ):
		print('Ligar a resistencia')
		relay_pin.write(0)
		switch1= 1
	if (maior_concreto > (targetTemp- 0.5)):
	...
	if (count > 6):
		count= 0
   		try:
   			response = channel.update({1:termopar1, 2:termopar2, 3: termopar3, 4: 
termopar4, 
5:ciclo, 6: str(ifaces),7: switch1, 8:targetTemp})
	    		print response
   		except:
   	    		sys.stderr.write("Thingspeak Exception")
